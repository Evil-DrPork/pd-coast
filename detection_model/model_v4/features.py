"""Deterministic, leakage-resistant features for the Poker44 v4 challenger.
...
def feature_implementation_sha256() -> str:
    """Fingerprint every repository file that can change the feature matrix."""
    project_root = Path(__file__).resolve().parents[2]
    dependencies = (
        Path(__file__).resolve(),
        project_root / "detection_model" / "model_v3" / "features.py",
        project_root / "detection_model" / "model_v3" / "schema.py",
        project_root / "poker44" / "validator" / "payload_view.py",
    )
